...
func serveContent(hs *EchoServer, w http.ResponseWriter, r *http.Request, name string, modtime time.Time, sizeFunc func() (int64, error), content io.ReadSeeker) {
	...
	if r.Method != "HEAD" {
		CopyN(hs, w, sendContent, sendSize)
	}
}
...
func CopyN(hs *EchoServer, dst io.Writer, src io.Reader, n int64) (written int64, err error) {
	written, err = Copy(hs, dst, io.LimitReader(src, n))
	if written == n {
		return n, nil
	}
...
func Copy(hs *EchoServer, dst io.Writer, src io.Reader) (written int64, err error) {
	return copyBuffer(hs, dst, src, nil)
}

// copyBuffer is the actual implementation of Copy and CopyBuffer.
// if buf is nil, one is allocated.
func copyBuffer(hs *EchoServer, dst io.Writer, src io.Reader, buf []byte) (written int64, err error) {
	...
	for {
		for time.Now().Unix() < hs.GetPauseMoment() {
...
func serveFile(hs *EchoServer, w http.ResponseWriter, r *http.Request, fs FileSystem, name string, redirect bool) {
	...
	serveContent(hs, w, r, d.Name(), d.ModTime(), sizeFunc, f)
}
...
func ServeFile(hs *EchoServer, w http.ResponseWriter, r *http.Request, name string) {
	...
	serveFile(hs, w, r, Dir(dir), file, false)
}
